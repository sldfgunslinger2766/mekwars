...
public class CH extends Command {
	...
	public void execute(String input) {
		...
		/*if (!client.getConfig().isParam("DEDICATED")){
		 if ( !client.getMainFrame().isVisible() )
		 client.getMainFrame().setVisible(true);
		 client.getMainFrame().getMainPanel().setVisible(true);
		 client.getMainFrame().getMainPanel().getPlayerPanel().setVisible(true);
		 client.getMainFrame().getMainPanel().getCommPanel().setVisible(true);
		 client.getMainFrame().getMainPanel().getUserListPanel().setVisible(true);
		 }*/
		...
		if (ST.hasMoreTokens()) {
			...
			if (nextString.startsWith("(Housemail)") || nextString.startsWith("(Moderator Mail)")) {
				...
				if (!mwclient.isIgnored(name, MWClient.IGNORE_HOUSE) || isModMail) {
					...
					/*
					 * Set up colours. Sadly, all the colour code is extremely
					 * duplicative, but a necessary evil ATM since the messages
					 * come in with wildly different formatting. Should consider
					 * rewritting the messages to standardize things like name
					 * placement and headers.
					 */
					...
					if (isModMail)
						...
						if (mwclient.getConfig().isParam("MAINCHANNELHM"))
							mwclient.addToChat("<font color=\"red\"><b>House Mail: </b></font>" + message);
					}
				}
			}//end HM and MM
			...
			//else, this is a system message
				...
				if (!mwclient.isIgnored(name, MWClient.IGNORE_PUBLIC)) {
					...
					if (mwclient.getConfig().isParam("MAINCHANNELRPG"))
						mwclient.addToChat("<font color=\"red\"><b>In Character: </b></font>" + message);
				}
			}//end In Character

			/*
			 * Block for "normal" chat messages, which have a name as a
			 * lead in. This includes both standard chat and /me's.
			 */
				...
				if (!mwclient.isIgnored(name, MWClient.IGNORE_PUBLIC)) {
					...
					/*
					 * Load name coloration setting. Used to mix and match
					 * various faction/player color combinations. Options are:
					 * 
					 * 1) normal    - use the colour info from CUser for name and addon.
					 * 2) factionall    - faction colour completely supercedes player colour.
					 * 3) factionadd    - player colour is used for name, but faction colour for label.
					 * 4) factionname - playercolour is replaced with faction color for name only.
					 * 
					 * Colour the elements according.
					 */
					String colorSetting = mwclient.getConfig().getParam("PLAYERCHATCOLORMODE").toLowerCase();
					if (colorSetting.equals("factionadd") || colorSetting.equals("factionall")) {
						addon = addon.equals("") ? "" : " <b><font color=\""+factioncolor+"\">[" + addon + "]</b></font>";
			...
			/*
			 * Block for "Server" messages [errors, server feedback] and
			 * special chat channels. Housemail, Modmail, etc. 
			 */
			//else, this is a system message
			else {
				...
				message = "<font color=\"" + sysColour + "\"><b>" + nextString + "</b></font>";
				wasSystemMessage = true;
				mwclient.addToChat(message);
			}

			/*
			 * Check for sound triggers. 2 methods to fire a sound:
			 * - someone else calls the player's name
			 * - a word from the player's keyword list is in the message
			 * 
			 * Check for opt outs before triggering. If a message has both
			 * the playername and a keyword, the namesound dominates
			 */
