...
public class UnitViewerDialog extends JFrame implements ActionListener, KeyListener, ListSelectionListener, Runnable, WindowListener, ItemListener {
	...
	public UnitViewerDialog(CMainFrame cl, UnitLoadingDialog uld, MWClient mwclient, int viewer) {
		...
		mechList.setVisibleRowCount(17);//give the list same number of rows as the text boxes
	...
	private void filterMechs(boolean calledByAdvancedSearch) {
		...
		try {
            for (int x = 0; x < mechs.length; x++) {
                /*
                 * A hacky check which prevents errors units from being
                 * added to the unit viewer lists, leaving only "valid"
                 * units from the dataset. 
                 */
    ...
    private void sortMechs() {
        ...
        //mechList.setPreferredSize(new Dimension(180,mechsCurrent.length * 19));
        repaint();
    }
	...
	private String formatMech(MechSummary ms) {
		...
		makeLength(Integer.toString(ms.getTons()), 3) + " " + 
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == bSelect){
			this.saveComboBoxSettings();
		    if ( this.viewerType == UnitViewerDialog.OMNI_VARIANT_SELECTOR ){
				try{
	...
	void previewMech(Entity entity) {
        ...
        paintScreen(entity != null && currEntity.getFluff() != null
                && viewFluff );
