...
public class RepodSelectorDialog extends JFrame implements ActionListener, KeyListener, ListSelectionListener, Runnable, WindowListener, ItemListener {
	...
	public RepodSelectorDialog(CMainFrame cl, UnitLoadingDialog uld, MWClient mwclient, String chassieList, String unitId) {
		...
		while (ST.hasMoreElements()){
		    ...
		    if ( tempstr.equals("GLOBAL") )
		        ...
		        //MWClient.mwClientLog.clientErrLog("Chassie: "+tempstr+" mods: "+chassieMods+" ChassieList: "+chassieList);
		...
		mechList.setVisibleRowCount(22);//give the list same number of rows as the text boxes
	...
	private void filterMechs() {
		...
		try {
			for (;x < mechs.length; x++) {
				...
				if ( chassieList.get(mechs[x].getName()) != null && !vMechs.contains(mechs[x].getName())) {
	...
	private String formatMech(MechSummary ms) {
		...
		String chassieMods = (String)chassieList.get(ms.getName());
		//MWClient.mwClientLog.clientErrLog("Name: "+ms.getName()+" Mods: "+chassieMods);
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == bRepod){
			try{
				...
				if (global)
				    mwclient.sendChat(MWClient.CAMPAIGN_PREFIX + "c repod#"+unitId+"#GLOBAL#"+ms.getName());
				else
				    mwclient.sendChat(MWClient.CAMPAIGN_PREFIX + "c repod#"+unitId+"#"+ms.getName());
				Thread.sleep(125);
				this.dispose();
			}
			catch(Exception ex){
