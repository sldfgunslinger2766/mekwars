...
public class BlackMarketPartsModel extends AbstractTableModel {
	...
	public Object getValueAt(int row, int col) {
		if (row < 0) return "";
		if (row >= sortedComponents.length) return "";
	...
	class Renderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
			...
			c.setOpaque(true);
			if ( components.size() < row || row < 0 ) return c;
			if (table.getModel().getValueAt(row, column) != null) {
				c.setText(table.getModel().getValueAt(row, column).toString());
			}
			c.setToolTipText("");

			BMEquipment bme = components.get(table.getModel().getValueAt(row, BlackMarketPartsModel.PART));
