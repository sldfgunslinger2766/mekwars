...
public abstract class CGUICommand extends AbstractAction implements IGUICommand
{
  ...
  {
    if (tname.startsWith(GUIprefix)) {tname = tname.substring(GUIprefix.length());}
  ...
  protected void send(String input) {Connector.send(prefix + command + delimiter + input);}

  // send command to server (directly through connector)
  protected void send() {Connector.send(prefix + command);}

  // remove prefix and name/alias from input
  protected String decompose(String input)
  {
    if (input.startsWith(GUIprefix)) {input = input.substring(GUIprefix.length()).trim();}
    if (input.startsWith(name)) {input = input.substring(name.length()).trim();}
    else if (input.startsWith(alias)) {input = input.substring(alias.length()).trim();}
    return input;
  }

  public String getName() {return name;}
  public String getAlias() {return alias;}
  public boolean isAlias() {return (alias != null && !alias.equals(""));}
}