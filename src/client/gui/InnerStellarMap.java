...
MouseMotionListener, MouseWheelListener, ActionListener {
	...
	static public final class InnerStellarMapConfig {
		...
		/**
		 * brightness correction for colors.
		 * This is no gamma correction! Gamma correction brightens medium level
		 * colors more than extreme ones.
		 * 0 means no brightening.
		 */
		 ...
		 * Names, Control, Factories, Warehouses, Ranges, Changes
		 */
	 ...
	 * ALL, _FILLER_, Factories, Facilities, Disputed, Contested
	 */
	...
	/**
	 * A data stucture to hold all planets marked as "changed" since last update.
	 * - see common.CampaignData.decodeMutablePlanets()
	 */
	private Map<Integer,Influences> changesSinceLastRefresh;
	
	/**
	 * Used to indicate the blinking of a planet.
	 * If true, the planets are drawn white.
	 */
	private boolean blinkPhase = false;
	
	/**
	 * Method which returns the panel underlying the map. This is used
	 * by the AdminMapPopupMenu class in admin package.
	 */
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getButton() == MouseEvent.BUTTON3) {
				...
				public void actionPerformed(ActionEvent ae) {
					...
					if (Boolean.parseBoolean(mwclient.getserverConfigs("UseStaticMaps"))) {
						...
						JOptionPane.showMessageDialog(
								InnerStellarMap.this, new JScrollPane(label),
			...
			/*
			 * If a planet is selected and the player is active, build
			 * an attack menu and include it in the popup.
			 */
			if (p != null) {
					...
					if (!loadJar.exists())
		        		...
		        		c.getDeclaredMethod("createMenu", 
		        				new Class[] {MWClient.class, InnerStellarMap.class, Integer.class, Integer.class, Planet.class}).invoke(o,
		        						new Object[] {mwclient, this, new Integer((int)scr2mapX(e.getX())), new Integer((int)scr2mapY(e.getY())), mp.getPPanel().getPlanet()});
		        		popup.add((JMenu)o);
					}
	        	} catch (Exception ex) {
	 ...
	 * 
	 * If anyone has a better algorithm than this stupid kind of shit, please,
	 * feel free to exchange my brute force thing...
	 * 
	 * An good idea would be an voronoi diagram and the sweep algorithm from
	 * Steven Fortune.
	 */
	...
	public void paint(Graphics g) {
        ...
        if(conf.display[DISPLAY_OVERLAY] && overlayLines != null) {
            for(ArrayList<Position> points : overlayLines) {
                g.setColor(StringUtils.html2Color(mwclient.getConfigParam("MAPOVERLAYCOLOR")));
                Position last = null;
		...
		for ( Planet p : planets){
             ...
             * TODO: Checking on every pait is slow. We should
             *       cache the visibility data in a map of some kind
             *       and clear it on refresh or viewing option change.
             */
            if (!this.planetIsVisible(p))
                continue;
			...
			if (mp.getPPanel() != null && mp.getPPanel().getPlanet() != null && 
			...
			int dotSize = size;
			boolean blink = false;
			if (conf.display[DISPLAY_LAST_CHANGED] && blinkPhase && 
			...
			if (!blink)
				g.setColor(c);
			if (conf.display[DISPLAY_NAMES] && (conf.showPlanetNamesThreshold == 0 || 
			...
			//unit factories
			if (conf.display[DISPLAY_UNITS] && (conf.showUnitFactoriesThreshold == 0 || 
			...
			if (conf.display[DISPLAY_RANGES]) {
				...
				for (String typeName : legalOps) {
					...
					if (range < 2000) {	
						...
						/*
						 * TODO: Replace the constant visibility checks with
						 *       a map. See earlier comment.
						 */
						if (!this.planetIsVisible(p))
			                continue;
	...
	/*private String getBestFactory(Planet p){
		...
		while (i.hasNext()) {
			...
			//Found an Assault Mek Factory not going to get any better then that!
			if ( type == Unit.MEK && weight == Unit.ASSAULT)
				return ("am");
		}
		
		//no factories redundent check.
		if ( type == Integer.MAX_VALUE && weight == Integer.MIN_VALUE)
			return "";
		
		switch ( weight ){
		case Unit.LIGHT: id="l";break;
		case Unit.MEDIUM: id="m";break;
		case Unit.HEAVY: id="h";break;
		case Unit.ASSAULT: id="a";break;
		}
		
		switch ( type ){
		case Unit.MEK: id += "m";break;
		case Unit.VEHICLE: id += "v";break;
		case Unit.INFANTRY: id += "i";break;
		}
		return id;
	}*/
	
	/**
	 * What we NOT want, is to wash out the color tone by adding simple
	 * gray to the color.
	 *
	 * I preferre the code from Color.brighter() which simple looks good.
	 * (But it had to be adjusted a bit)
	 * Imi
	 */
	...
	public void mouseDragged(MouseEvent e) {
		if (mouseMod != MouseEvent.BUTTON3) return;
	 ...
	 * - Called by the HyperLinkListener
	 */
	...
	/**
	 * Solves events of data fetches by adding the changes to the current 
	 * change set.
	 */
	...
	/*
	 * Called from action listeners, or stand alone
	...
	/**
	 * Method which saves current map properties. Called when
	 * a new planet is selected. The settings are restored when
	 * a client is loaded, preserving map selections between
	 * user sessions.
	 */
	...
	private boolean planetIsVisible(Planet p) {
		...
		/*
		 * Not showing all, so do check all relevant server
		 * options and determine whether this particular world
