...
public class CBMPartsPanel extends JPanel {
	...
	{
		bme = null;
		String part = (String)this.tblMarket.getModel().getValueAt(row, BlackMarketPartsModel.PART);
		
		if (part != null) {
			bme= (BMEquipment)theCampaign.getBlackMarketParts().get(part);
		}
		return bme;
	}
	...
	private void btnBuyPartsPerformed(ActionEvent evt) {
		...
		if (bme != null) {
			...
			String playerAmountString = JOptionPane.showInputDialog(null, "<HTML><center>How many units of " + bme.getEquipmentName() + " would you like to buy?","Amount to Buy", JOptionPane.PLAIN_MESSAGE);
            ...
            try {
            	...
            	//Trap the error
            	JOptionPane.showConfirmDialog(this, "Invalid Syntax Try Again.");
            	return;
            }
