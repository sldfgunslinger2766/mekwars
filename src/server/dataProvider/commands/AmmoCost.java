 ...
 */
public class AmmoCost implements ServerCommand {
    ...
    public void execute(Date timestamp, BinWriter out, CampaignData data)
            ...
            FileInputStream configFile = new FileInputStream("./campaign/ammocosts.dat");
            BufferedReader config = new BufferedReader(new InputStreamReader(configFile));
            
            while (config.ready()) {
                out.println(config.readLine(),"AmmoCost");
            }
        }
}
