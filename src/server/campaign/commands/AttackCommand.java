...
/**
 * AttackCommand is used to initiate
 * ShortOperations. Checks the validity
 * of the attacking force, checks to
 * ensure that the defenders available
 * to the attacking force can defend the
 * target op type, and then creates the
 * operation and gives attackers their
 * "Special" options.
 */
public class AttackCommand implements Command {
	...
	public void process(StringTokenizer command,String Username) {
		...
		try {
			...
			CampaignMain.cm.toUser("No operation name given. Try again.",Username,true);
			return;
		}
		...
		if (aa == null) {
			CampaignMain.cm.toUser("An error occured while creating your Army (The Army was null. This usually means " +
		...
		try {
			...
			CampaignMain.cm.toUser("No planet name given. Try again.",Username,true);
			return;
		}
		...
		if ( o.getBooleanValue("TeamOperation") && o.getIntValue("NumberOfTeams") > 1) {
			...
			//Ok we got the attack and its valid. lets check to see if its a team Faction game if so add all active faciton Members
	        	...
	        	for ( String player : attackingFaction.getActivePlayers().keySet() ) {
	        		...
	        		for ( SArmy army : joiningPlayer.getArmies() ) {
						...
						toSend.append("<a href=\"MEKWARS/c joinattack#" + ap.getName()  + "#" + army.getID()+ "\">Army #"
					            + army.getID() + "</a> (Units: " + army.getUnits().size() + " / BV: " + army.getBV() + ")");
			...
			CampaignMain.cm.toUser("You have been assigned to team #"+teamNumber, Username);
