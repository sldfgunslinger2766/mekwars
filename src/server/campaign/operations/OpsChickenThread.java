...
public class OpsChickenThread extends Thread {
	...
	public synchronized void doPenalty() {
		...
		if (pdefender.getMyHouse().isNewbieHouse()) {
			String toPlayer = "You did not defend Attack #" + opID + ". You've not been punished because " +
		...
		if (expPercent > 0)
			totalEXPLoss = (int)(pdefender.getReward() * expPercent);
		if (fluPrecent > 0)
			totalFluLoss = (int)(pdefender.getReward() * fluPrecent);
		if (moneyPercent > 0)
			totalMoneyLoss = (int)(pdefender.getReward() * moneyPercent);
		...
		if (totalRPLoss > pdefender.getReward())
			totalRPLoss = pdefender.getReward();
		if (totalEXPLoss > pdefender.getExperience())
			totalEXPLoss = pdefender.getExperience();
		if (totalFluLoss > pdefender.getInfluence())
			totalFluLoss = pdefender.getInfluence();
		if (totalMoneyLoss > pdefender.getMoney())
			totalMoneyLoss = pdefender.getMoney();
		...
		/*
		 * Player has been aprised of his troubles ... now let the
		 * whole world know that the house was spanked. And, just for
		 * fun, reveal the player's name. Shame is sometimes a better
		 * hinderance than the actual punishment ...
		 */
		...
		String toMain = pdefender.getColoredName() + " did not defend " + StringUtils.aOrAn(attackH.getName(),true,false)
		    + attackH.getColoredNameAsLink() + " attack on " + target.getNameAsColoredLink() + "(#" + opID + ") in time. "
		    + defendH.getColoredNameAsLink() + " lost ";
		...
		/*
		 * Note that unlike the old task system which looked for factories
		 * and took from them, or took light units if no factories were
		 * on world, Operations takes the BEST units available at the time.
		 */
		if (unitPenalty > 0) {
		...
		/*
		 * Note that unlike the old task system which looked for factories
		 * and took from them, or took light components if no factories were
		 * on world, Operations takes the BEST Mek comps available at the time,
	...
	public synchronized void run() {
		...
		/*
		 * Chicken loop. shouldContinue is set to false
		 * when a player makes an external action which
		 * should prevent chickening:
		 *  
		 *  - joins this game
		 *  - becomes invovled in another RUNNING game
		 *  - deactivates
		 *  
		 * Deactivating will pull the players chicken
		 * threads from the manager, looping through
		 * to stop them. It will also punish the player
		 * for leeching, calling the public doPenalty()
		 * in order to give a punishment equal to that
		 * delivered for a timeout. 
		 */
		while(true) {
			 ...
			 * deactivation process will, itself, cause a leech penalty, so
			 * we don't actually need to do one.
			 * 
	...
	public String generateAttackLinks() {
		/*
		 * Setup. Inform the player of the outstanding
		 * attack and send him links to join for defense.
		 */
		Operation o = CampaignMain.cm.getOpsManager().getOperation(opName); 
		boolean hasAnArmy = false;//used to set up string for armies past the first
		...
		for (SArmy currArmy : parmies) {
			...
			//Team Operations that are faction specific get teams assigned in the defendCommand
			if ( o.getBooleanValue("TeamOperation") 
				...
				for ( int teamNumber = 1; teamNumber <= numberOfTeams; teamNumber++)
					toSend.append("<a href=\"MEKWARS/c defend#" + opID  + "#" + aID+"#"+teamNumber+ "\"> Team #"+teamNumber+" Army #"
							+ aID + "</a> (Units: " + aUnits + " / BV: " + aBV + ")");
			}else {
				toSend.append("<a href=\"MEKWARS/c defend#" + opID  + "#" + aID+"#"+-1+ "\">Army #"
			            + aID + "</a> (Units: " + aUnits + " / BV: " + aBV + ")");
			}
