...
class ReaderThread extends Thread {
    ...
    private void inflate(String command) throws Exception {
         ...
         * MWDedHost.MWDedHostLog.clientOutputLog("\t...Checksum of /deflated is " +
         ...
         * //MWDedHost.MWDedHostLog.clientOutputLog("\tfirst bytes are " +
         * sb2.toString()); // ... still more String s;
         * //MWDedHost.MWDedHostLog.clientOutputLog("Probably useless /deflate
         * payload follows:"); while ((s = cbr.readLine()) != null) {
         * MWDedHost.MWDedHostLog.clientOutputLog("\t" + s); }
         */
        inflater.reset();
        ...
        try {
            ...
            while (keepGoing) {
            	...
            	if (_listener != null) {
                    ...
                    MWDedHost.MWDedHostLog.clientErrLog("Null listener: " + newLine);
                }
            }
            MWDedHost.MWDedHostLog.clientErrLog("ReaderThread: stopping gracefully.");
            
        } catch (IOException e) {
            if (keepGoing) {
            	pleaseStop();
                MWDedHost.MWDedHostLog.clientErrLog("ReaderThread Error");
