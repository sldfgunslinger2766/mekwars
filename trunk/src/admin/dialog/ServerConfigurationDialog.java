...
public final class ServerConfigurationDialog implements ActionListener {
	...
	public ServerConfigurationDialog(MWClient mwclient) {
		 ...
		 * 
		 * Set up the PGMH panel, which indicates where HTML output (EXPRanking, etc.) is
		 * dumped and where certain core server files are located. 
		 */
		 ...
		 * 
		 * Influence panel, where admins set influence gain controls (bv limits, etc)
		 * and action costs (bm bid, attack, and so on).
		 * 
		 * Use nested layouts. A Box containing a Flow, which in turn contains two Springs
		 */
        ...
        baseTextField.setToolTipText("<HTML>" +
				"Multiplier used to determine how much additional XP a SOL<br>" +
				"player must earn to join a low ranked faction. XP to Defect is<br>" +
				"[Min To Defect] + [Rank of Target House * XP per faction Rank]</HTML>");
		...
		/*
		 * DEFECTION PANEL CONSTRUCTION
		 * 
		 * Panel which controls most defection-related matter. Some
		 * SOL-specific things handled in Newbie panel.
		 */
        ...
        BaseCheckBox.setToolTipText("<HTML>" +
        		"If both this box and \"Reset Leaving SOL\" are checked, players will<br>" +
        		"receive faction units drawn from their new faction's tables instead of<br>" +
        		"of SOL units on defection. Units will be taken from the 3025 era ONLY.</HTML>");
		...
		/*
		 * MISC PANEL CONSTRUCTION
		 * 
		 * options that are difficult to categorize ...
		 */
		...
		/*
		 * ARTILLERY TAB CONSTRUCTION
		 * 
		 * Enable autoassigned artillery, and set up
		 * loadout options.
		 */
		...
		/*
		 * NEWBIE HOUSE PANEL CONSTRUCTION.
		 * 
		 * Set units SOL receives and units which are legal vs SOL
		 */
        ...
        baseTextField.setToolTipText("<HTML>" +
				"SOL player needs to have this many (or<br>" +
				"fewer) units to trigger a hangar reset</HTML>");
        ...
        baseTextField.setToolTipText("<HTML>" +
				"Number of resets a SOL player is allowed<br>" +
				"after a game, while immune from attack." +
				"</HTML>");
        ...
        BaseCheckBox = new JCheckBox("Selectabel Salvage");
        BaseCheckBox.setToolTipText("Should salvage be selectable?");
        ...
        baseTextField.setToolTipText("<html>" +
				"Number of disconnecting players' units ejected,<br>" +
				"in addition to those already marked as salvage<br>" +
				"by the in-game status updates.</html>");
        ...
        baseTextField.setToolTipText("<html>" +
				"Amount of time added (sec) back to disconnection counter when<br>" +
				"a player returns. For example, if there are 10 minutes<br>" +
				"before a report and a grace period of 3 minutes, a player<br>" +
        ...
        baseTextField.setToolTipText("<html>" +
				"Percentage of normal pay (0-100) given to players<br>" +
				"who disconnect. Loser modifiers are applied normally.</html>");
         ...
         * 
         * defaults.setProperty("", "0");//int
        defaults.setProperty("", ".50");//float
        defaults.setProperty("", ".50");//float
        defaults.setProperty("", "1.0");//float
        defaults.setProperty("", "1.0");//float
        defaults.setProperty("", "1.0");//float
        defaults.setProperty("", "1.0");//float
        defaults.setProperty("", "1.0");//float
        defaults.setProperty("", "1.0");//float
        defaults.setProperty("", "0");//int. 0 ensures no payment by default.
         */
        ...
        baseTextField.setToolTipText("<html>" +
        		"Percentage of the cost of a similar new unit to the added to the Base<br>" +
        		"Payment. For example, if Base is 10 and a unit costs 40 CBills w/ a Variable<br>" +
        		"Loss Payment of 0.50 the starting compensation (before multipliers and<br>" +
        		"caps) will be (40 * .5) = 20 + 10 for a total of 30 CBills. float value</html>.");
        ...
        baseTextField.setToolTipText("<html>" +
				"Multiplier applied to loss compensation (base + variable) if unit in question is a mek.<br>" +
				"Example 1: Mek w 40 CBill base + var is destroyed * 1.25 mek multi = 50 CBills loss comp.<br>" +
				"Example 2: Mek w 40 CBill base + var is destroyed * 0.5 mek multi = 20 CBills loss comp.<br>" +
				"Note: Other multis and caps are also applied/checked. float value.</html>");
        ...
        baseTextField.setToolTipText("<html>" +
				"Multiplier applied to loss compensation (base + variable) if unit in question is a vehicle.<br>" +
				"Example 1: Veh w 20 CBill base + var is destroyed * 1.25 mek multi = 25 CBills loss comp.<br>" +
				"Example 2: Veh w 20 CBill base + var is destroyed * 0.5 mek multi = 10 CBills loss comp.<br>" +
				"Note: Other multis and caps are also applied/checked. float value.</html>");
        ...
        baseTextField.setToolTipText("<html>" +
        		"Max amount of compensaton, expressed as a multiple of the cost of a similar<br>" +
        		"new unit from the player's faction. The Flat cap is checked AFTER the multiple cap.<br><br>" +
        		"Example 1: A new light mek costs 20 CBills. Base comp is 10 and var comp is<br>" +
        		"set to .50, giving a base + var of 20 CBills. The Multiple Cap is .75 of a<br>" +
        		"new unit's cost (20 * .75 = 15), so the compensation is capped at 15 CBills.<br>" +
        		"<br>" +
        		"Example 2: A new assault mek costs 100 CBills. Base compensation is 0 and var comp<br>" +
        		"is .60, for a base + var of 60 CBills. There's also a Mek Multiplier if 1.50, which<br>" +
        		"boosts compensation to 90 CBills. The Multiple Cap is .80 (100 * .80 = 80), so the<br>" +
        		"compensation is reduced to 80 CBills.</html>");
        ...
        baseTextField.setToolTipText("<html>" +
        		"Max amount of compensaton, expressed as a a simple integer. The Flat cap is checked<br>" +
        		"AFTER the multiple cap. Battle Loss Compensation can be disabled by setting a flat cap<br>" +
        		"of 0 CBills.<br><br>" +
        		"Example 1: A assault light mek costs 100 CBills. Base comp is 10 and var comp is<br>" +
        		"set to .50, giving a base + var of 60 CBills. The Flat Cap 40, so the compensation is<br>" +
        		"reduced to 40 CBills.<br>" +
        		"<br>" +
        		"Example 2: A new light mek costs 20 CBills. Base compensation is 10 and var comp<br>" +
        		"is .50, for a base + var of 20 CBills. The flat cap is 40 CBills. Unless there's<br>" +
        		"another multiplier (mek, salvage) or the payment is reduced by the Multiplier Cap,<br>" +
        		"the player will receive more money (40 CBills) in loss comp than it cost to buy his<br>" +
