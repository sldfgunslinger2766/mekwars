...
public class CHQPanel extends JPanel {
	...
	/**
	 * Public call which reinitializes the HQ panel.
	 * Hacky and evil, but lets camo and # columns in
	 * HQ display get updated on the fly.
	 */
	...
	private void createMeksPanel()
	{
		...
		for (int i = 0; i < tblMeks.getColumnCount(); i++) {
			tblMeks.getColumnModel().getColumn(i).setCellRenderer(this.MekTable.getRenderer());
		}
		...
		this.makeButtons();//makes pnlMeksBtns
	...
	class MechTableMouseAdapter extends MouseInputAdapter implements ActionListener {
		...
		public void mousePressed(MouseEvent e) {
			...
			if (dragUnit != null && e.getButton() == MouseEvent.BUTTON1) {
				...
				offset = new Point(28,22);//TODO: Make this a real offset, not a simple re-centering.
				...
				//save the image, drawn from mechinfo, to use as a drag under-image
			...
			/*
			 * and ... check to see if this should
			 * trigger a popup.
			 */
			maybeShowPopup(e);
		}
		...
		public void mouseReleased(MouseEvent e) {
			...
			/*
			 * If this was a drag, try to drop the
			 * unit into a target army or the hangar.
			 */
			if (isDrag) {
		...
		public void mouseDragged(MouseEvent e) {
			...
			if (isDrag) {
				 ...
				 * dragging a unit into an army which already contains the
				 * unit, mark ineligible. Else, show the drag cursor.
				 */
				int row = tblMeks.rowAtPoint(e.getPoint());
				int col = tblMeks.columnAtPoint(e.getPoint());
		...
		/**
		 * Private method called on click and release. Checks to see if
		 * if mouse event should open a contextual menu (right click, OS X
		 * control+click, etc) and shows a popup menu if appropriate.
		 */
		private void maybeShowPopup(MouseEvent e) {
			...
			if (e.isPopupTrigger()) {
				...
				if (col == 0 && row >= MekTable.getRowsForArmies()) {
					...
					//Choices [note - this array must be duplicated in CPlayer's sortHangar()]
					...
					if (l != null) {	
						...
						if (l.getBV() > 0) {
							...
							/*
							 if (!l.isReady()){
							 menuItem = new JMenuItem("Set Active");
							 menuItem.setActionCommand("SA|"+lid);
							 menuItem.addActionListener(this);
							 popup.add(menuItem);
							 }
							 else
							 {
							 menuItem = new JMenuItem("Set Inactive");
							...
							if (mwclient.getMyStatus() != MWClient.STATUS_ACTIVE && !canCheckFromReserve) {menuItem.setEnabled(false);}
							popup.add(menuItem);
					...
					/*
					 * CONSTRUCT the ADD menu here. It will be added to the actual format later.
					 * @urgru 12/7/04
					 */
						...
						if (mechArray.length > 0) {
							...
							for(int i = 0; i < mechArray.length; i++) {
								...
								if (Player.getAmountOfTimesUnitExistsInArmies(mm.getId()) >= Integer.parseInt(mwclient.getserverConfigs("UnitsInMultipleArmiesAmount")))
									continue;
								if (l.getUnit(mm.getId()) == null) {//only add if unit isn't already in army
									hasUnitsFree = true;
							...
							for (int i = 0; i < SubMenus.size();i++) {
								...
								if (SizeMenu.size() > 10) {
									...
									for (int j = 0; j < iterations;j++) {
										...
										} else {//BA, can assume this is i == 5.
										...
										//if adding proto or infantry menu, check previous elements
								...
								else {//Only one menu for the given size class is needed
					...
					if (cm != null) {
						 ...
						 * It will be added to the actual format later.
						 * @Torren 12/19/04
						 */
						...
						if (l.getUnits().size() > 0 && !l.isLocked()) {
								...
								while ( c3M.hasMoreElements()) {
								    ...
								    if (cm.getC3Level() != Unit.C3_IMPROVED){							        
								        if ( (c3Unit.getC3Level() == Unit.C3_MASTER || c3Unit.getC3Level() == Unit.C3_MMASTER)
								...
								/*if ( cm.getC3Level() == CUnit.C3_MASTER ){
								    ...
								    menuItem = new JMenuItem("Set as Company Commander");
								    menuItem.setActionCommand("LCN|"+lid+"|"+ cm.getId() +"|"+cm.getId());
								    menuItem.addActionListener(this);
								    linkMenu.add(menuItem);
								}*/
						}//end Link menu contruction
						
						//Link menu has been preformed. Proceed with the usual bits.
						mid = cm.getId();
						...
						if (mwclient.getPlayer().getHangar().size() > 0 && !l.isLocked()) {
							...
							/*
							 * The POSITION menu. Moves units around -within- the army. Only
							 * shown if there are enough units to warrant movement (>1).
							 */
					...
					//The ADD menu for empty spaces (null mech in an unlocked army)
					else
						popup.add(addMenu);
					...
					if (cm != null) {
						...
						if (ppqsEnabled && (cm.getType() == Unit.MEK || cm.getType() == Unit.PROTOMEK)) {
							...
							//option to remove pilot, if that hasn't been done already
							if (!cm.hasVacantPilot()){
                                pm.addSeparator();
							...
							//else, the pilot is vacant, so give an "add" option
							//in leiu of the normal exchange pilot option
							else
								jm = new JMenu("Assign");
					
							/*
							 * Set up the actual menu, *IF* the
							 * pilot queue has a non-zero size.
							 */
							if (pilots.length == 0)
								jm.setEnabled(false);
							else {
								...
								/*
								 * Contruction of EXCHANGE pilot. Derived
						...
						/*
						 * Disable RFAA option if unit isnt actually
						 * IN any of the player's armies.
						 */
						...
						if (hangernum == mwclient.getPlayer().getHangar().size()) {//only show in first free cell
                            if ( useAdvanceRepairs ){
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (command.equalsIgnoreCase("EXM")) {
                ...
                for ( CArmy currArmy : mwclient.getPlayer().getArmies()){
                    ...
                    if (toSend.trim().equals(""))
                        toSend = "Looking for a game at";//matches default config
    				...
    				//BV and class info
                        ...
                        while (e.hasMoreElements()) {
                            ...
                            case Unit.ASSAULT: assault++; break;
                            case Unit.LIGHT: light++; break;
                            case Unit.MEDIUM: medium++; break;
                            case Unit.HEAVY: heavy++;break;
                            }
                        }
                        if ( assault > 0)
                            toSend += " "+assault+"A,";
				...
				int mid = Integer.parseInt(st.nextToken());
				mwclient.sendChat(MWClient.CAMPAIGN_PREFIX + "c retirepilot#" + mid);//send directly
				//show mek
			...
			//Estimate Unit Repairs
                ...
                if ( Boolean.parseBoolean(mwclient.getserverConfigs("UseRealRepairCosts")) ){
                    ...
                    mwclient.systemMessage("It'll cost you at least the following to repair your "+mek.getModelName()+".<br><table>" +
                            "<tr><th>Green Tech:</th><th>"+mwclient.moneyOrFluMessage(true,true,(int)repairCost,false)+" in parts and "+mwclient.moneyOrFluMessage(true,true,greenTechCost,false)+" in labor for a total of "+mwclient.moneyOrFluMessage(true,true,(int)repairCost+greenTechCost,false)+".</th></tr>"+
                            "<tr><th>Reg Tech:</th><th>"+mwclient.moneyOrFluMessage(true,true,(int)repairCost,false)+" in parts and "+mwclient.moneyOrFluMessage(true,true,regTechCost,false)+" in labor for a total of "+mwclient.moneyOrFluMessage(true,true,(int)repairCost+regTechCost,false)+".</th></tr>"+
                            "<tr><th>Vet Tech:</th><th>"+mwclient.moneyOrFluMessage(true,true,(int)repairCost,false)+" in parts and "+mwclient.moneyOrFluMessage(true,true,vetTechCost,false)+" in labor for a total of "+mwclient.moneyOrFluMessage(true,true,(int)repairCost+vetTechCost,false)+".</th></tr>"+
                            "<tr><th>Elite Tech:</th><th>"+mwclient.moneyOrFluMessage(true,true,(int)repairCost,false)+" in parts and "+mwclient.moneyOrFluMessage(true,true,eliteTechCost,false)+" in labor for a total of "+mwclient.moneyOrFluMessage(true,true,(int)repairCost+eliteTechCost,false)+".</th></tr></table>");
                }else{
				...
				for (CArmy currA : mwclient.getPlayer().getArmies())
					if (currA.getUnit(mid) != null) {mwclient.sendChat(MWClient.CAMPAIGN_PREFIX + "c EXM#"+currA.getID()+","+mid);}
	...
	public class MekTableModel extends AbstractTableModel {
		...
		public int getRowsForHangar(){
			...
			if (freebays > 1) {freebays = 1;}
			if (freebays < 0) {freebays = 0;}
		...
		public Object getValueAt(int row, int col) {
			...
			if (col == 0) {
				...
				if (row < getRowsForArmies()) {
					...
					if ( useOpRule && army.getOpForceSize() > army.getUnits().size() )
						modifiedBV = "("+Integer.toString((int)(army.getBV()*army.forceSizeModifier((double)army.getOpForceSize())))+")";
			...
			if (cm == null && row < getRowsForArmies())
				...
				int hangernum = (row - this.getRowsForArmies()) * (getColumnCount() - 1)  + col - 1;
				if (hangernum == Player.getHangar().size()){//only show in first free cell
                    if ( useAdvanceRepairs )
		...
		public class Renderer extends MechInfo implements TableCellRenderer {
			...
			public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
				...
				Component c = this;
				this.setOpaque(true);
				this.setText(getValueAt(row, column).toString());
				...
				if (l != null) {
					if (column == 0) {
						...
						if (l.isLocked()) {c.setBackground(new Color(235,225,5));}
						return c;
					}
				} else if (column == 0) {
				...
				if (cm != null) {
					...
					if (cm.getStatus() == Unit.STATUS_FORSALE) {
						...
						if (cm.getPilot().getName().equals("Vacant")) {//RFE 1545928 - Color for pilotless units
							...
							if (scheme.equals("classic"))
								c.setBackground(new Color(65,170,55));//dark green
							else //all non-classic sets (light blue)
							...
							if(scheme.equals("tan")) {
								...
								case Unit.LIGHT: c.setBackground(new Color(dcolor.getRed() -10, dcolor.getBlue() -10, dcolor.getGreen() -30)); break;
								case Unit.MEDIUM: c.setBackground(new Color(dcolor.getRed() -30, dcolor.getBlue() -30, dcolor.getGreen() -50)); break;
								case Unit.HEAVY: c.setBackground(new Color(dcolor.getRed() -55, dcolor.getBlue() -55, dcolor.getGreen() -75)); break;
								case Unit.ASSAULT: c.setBackground(new Color(dcolor.getRed() -75, dcolor.getBlue() -75, dcolor.getGreen() -95)); break;
							...
							//GREY SET. Grey gradients.
								...
								case Unit.LIGHT: c.setBackground(dcolor); break;
								case Unit.MEDIUM: c.setBackground(new Color(dcolor.getRed() -17, dcolor.getBlue() -17, dcolor.getGreen() -17)); break;
								case Unit.HEAVY: c.setBackground(new Color(dcolor.getRed() -40, dcolor.getBlue() -40, dcolor.getGreen() -40)); break;
								case Unit.ASSAULT: c.setBackground(new Color(dcolor.getRed() -65, dcolor.getBlue() -65, dcolor.getGreen() -65)); break;
								...
								case Unit.LIGHT: c.setBackground(dcolor); break;
								case Unit.MEDIUM: c.setBackground(new Color(dcolor.getRed() -30, dcolor.getBlue() -30, dcolor.getGreen() -50)); break;
								case Unit.HEAVY: c.setBackground(new Color(dcolor.getRed() -65, dcolor.getBlue() -65, dcolor.getGreen() -25)); break;
								case Unit.ASSAULT: c.setBackground(new Color(dcolor.getRed() -45, dcolor.getBlue() -93, dcolor.getGreen() -45)); break;
					...
					int freebays = Player.getFreeBays();
					if (freebays < 0) {freebays = 0;}
					if (meknum > freebays + Player.getHangar().size()) {setText("");}
				}
				return c;
			}
		}//end Renderer
