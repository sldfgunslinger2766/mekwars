...
/**
 * This is a tabbed multi-channet Communications Panel
 * using Swing to manage the display.
 */
/*
...
public class CCommPanel extends JPanel implements ChangeListener, ComponentListener, MouseListener {
	...
	/**
	 * Maximum buffer of each channel. Each channel will be capped to have no more than this
	 * amount of characters.
	 */
	public static final int MAXBUFFER = 100000;
	/**
	 * Amount of characters removed when buffer overflow. If a channels maximum character count
	 * is reached, cap that many characters.
	...
	JPanel NChannelPanel = new JPanel();
	CSelectTabAction NChannelSelect = null;
	...
	public CCommPanel(MWClient client) {
		...
		/*
		 * Instead of making 6 HyperLinkListeners,
		 * make one here and use it for all tabs.
		 * 
		 * @urgru 7.17.05
		 */
		...
		if (mwclient.getConfig().isParam("HOUSEMAILVISIBLE")) {
			...
			CommTPane.addTab(tabText, null, HMailPanel,
			...
			getInputMap(WHEN_IN_FOCUSED_WINDOW).put(
					KeyStroke.getKeyStroke("alt "+mnemonicText.toUpperCase()), "HMailSelect");
		...
		if (mwclient.getConfig().isParam("RPGVISIBLE")) {
			...
			getInputMap(WHEN_IN_FOCUSED_WINDOW).put(
					KeyStroke.getKeyStroke("alt "+mnemonicText.toUpperCase()), "RPGChannelSelect");
		...
		if (!mwclient.getConfig().isParam("USEMULTIPLEPM")) {
			...
			if (mwclient.getConfig().isParam("PRIVATEMAILVISIBLE")) {
				...
				CommTPane.addTab(tabText, null, PMailPanel,
						"Private Mail (Alt + "+mnemonicText.toUpperCase()+")");
				...
				getInputMap(WHEN_IN_FOCUSED_WINDOW).put(
						KeyStroke.getKeyStroke("alt "+mnemonicText.toUpperCase()), "PMailSelect");
		...
		if(mwclient.getConfig().isParam("PERSONALLOGVISIBLE")) {
			...
			getInputMap(WHEN_IN_FOCUSED_WINDOW).put(
					KeyStroke.getKeyStroke("alt "+mnemonicText.toUpperCase()), "PLogSelect (Alt + "+mnemonicText.toUpperCase()+")");
		...
		if (mwclient.getConfig().isParam("SYSTEMLOGVISIBLE")) {
			...
			CommTPane.addTab(tabText, null, SLogPanel,
					"System Messages (Alt + "+mnemonicText.toUpperCase()+")");
			...
			getInputMap(WHEN_IN_FOCUSED_WINDOW).put(
					KeyStroke.getKeyStroke("alt "+mnemonicText.toUpperCase()), "SLogSelect");
		...
		MiscChannelEPane.addHyperlinkListener(new HyperlinkListener() {
			public void hyperlinkUpdate(HyperlinkEvent e) {
				/*if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED)
				 removeHttpLinksFromEditorPane(CHANNEL_MISC);*/
			}
		...
		/*CHANNEL_MAIN = CommTPane.indexOfComponent(MChannelPanel);
		 ...
		 CHANNEL_HMAIL = CommTPane.indexOfComponent(HMailPanel);
		 CHANNEL_PMAIL = CommTPane.indexOfComponent(PMailPanel);
		 CHANNEL_PLOG = CommTPane.indexOfComponent(PLogPanel);
		 CHANNEL_SLOG = CommTPane.indexOfComponent(SLogPanel);
		 CHANNEL_MISC = CommTPane.indexOfComponent(MiscChannelPanel);*/
		/*
		...
		/*
		 * Add Function Keys to the maps. Putting them here is fairly hacky; however,
		 * the tabbing controls (more forwward and back with x/z) are already here,
		 * so one more universal listen in CommPanel shouldn't kill anyone.
		 *
	...
	public boolean sendChat(String s) {
		...
		if (!s.startsWith(MWClient.GUI_PREFIX) && !mwclient.getConfig().isParam("USEMULTIPLEPM")
		...
		if (!s.startsWith(MWClient.GUI_PREFIX) 
				&& mwclient.getConfig().isParam("USEMULTIPLEPM")
				&& ((JPanel)CommTPane.getComponent(CommTPane.getSelectedIndex())).getName() != null
				&& ((JPanel)CommTPane.getComponent(CommTPane.getSelectedIndex())).getName().startsWith("Mail Tab ")) {
	...
	public void setChat(String s, int channel, String mailTab) {
		...
		if (scrollbar != null && !autoTextUpdate && (scrollbar.getValue()+scrollbar.getVisibleAmount()) < 
				scrollbar.getMaximum()*0.97) {
			scroll = false;
		...
		try {
			...
			((HTMLEditorKit) editorpane.getEditorKit()).read(
					new java.io.StringReader(s), editorpane.getDocument(),
	...
	/* remove in a given window all http-link tags */
	public void removeHttpLinksFromEditorPane(int channel) {
		JEditorPane ePane = getEditorPane(channel);
		ePane.setText(ePane.getText().replaceAll("(<a[^>]*>)|(</a>)", ""));
	...
	public void componentResized(ComponentEvent e) {
		for (int i = 0; i < CommTPane.getTabCount(); i++) {
			...
			editorpane.setCaretPosition(editorpane.getDocument().getLength()); // we document
			scrollbar.setValue(scrollbar.getMaximum() - scrollbar.getVisibleAmount()); // and scrollbar at bottom
		}
	}
		...
		public void keyPressed(KeyEvent e) {
			...
			if (e.getKeyCode() == KeyEvent.VK_UP) {
				...
				if (UserNumber == 0){
					...
					//Get rid of any leading spaces if the name is the first thing typed.
