 ...
 *
 * Campaign is the base of the data holding classes for client and server.
 * Here all campaign relevant information as Houses, Planets and Player data
 * is stored.
 * 
 * In this base class some methods are provided to retrieve these 
 * informations to use in common data classes like House or Planet when 
 * reffering to ressources.
 *
...
public class CampaignData implements TerrainProvider {
    ...
    /**
     * All different House ids are stored here
     * key=String (name), value=int id
     */
    private TreeMap<String,Integer> factionid = new TreeMap<String,Integer>();
    
    /**
     * This is a list with all planet information stored.
     * key=Integer (id), value=Planet (or subclasses for server and client)
     */
    private  TreeMap<Integer,Planet> planets = new TreeMap<Integer,Planet>();

    /**
     * This is a list with planet id stored.
     * key=String (name), value=int id
     */
    ...
    /**
     * Retrieve a planet by its name. Please try to use planet Id's when 
     * lookup for a planet instead (if you have the choice).
     */
    ...
    /**
     * @author jtighe
     * Retrieve a factory by its name.
     * 
     */
     ...
     * @param Factory
     * 
     * Updates the Client side factories
     * Useful for the factory Refresh with RP
     */
     ...
     * @param name
     * @return 
     * @TODO This seems to be only needed, because some serialization
    ...
    /**
     * Since I have no idea how TinyXML is operating and since McWizard does
     * not allow me to use my loved JDom and finally since Enkel does not like
     * XML-Transfer anyway, I use this to encode/decode the whole object..
     * (Imi)
     * 
     ...
     * 
     * TODO: check http://jira.codehaus.org/secure/ViewIssue.jspa?key=XSTR-27
     * to see whether a better way of serialization is available ;-)
     */
    ...
    public void binIn(TreeReader in, CampaignData dataProvider) throws IOException {
        ...
        for (int i = 0; i < size; ++i) {
            String type = in.readString("factionType");
            try {
        ...
        for (int i = 0; i < size; ++i) {
            String type = in.readString("planetsType");
            try {
    ...
    */
    ...
    /**
     * extracts data from the BinReader and places it into the client side
     * hash table.
     * @param in
     * @param userLevel
     */
    public void importAccessLevels(BinReader in){
        ...
        catch(Exception ex){}//in is empty move on.
