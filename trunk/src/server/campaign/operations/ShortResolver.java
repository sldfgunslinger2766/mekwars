...
public class ShortResolver {
	...
	/*
	 * These variables are NOT true instance variables. They're set
	 * when a resolve() is called - in a specific order - and used 
	...
	TreeMap<String, String> unitStrings;//salvage, level up and pilot recovery information.
	...
	TreeMap<String, Integer> lossCompensation;//reimbursement for destroyed or enemy-salvaged units.
	...
	TreeMap<String, OpsScrapThread> scrapThreads;//threads are put into OpsMan @ end. 
	...
	//METHODS
	/**
	 * Method which clears or nulls all of the holder
	 * variables. Private. Called when a resolve() is
	 * finished.
	 */
	...
	/** 
	 * Method which takes in an autoreport string,
	 * reads operation configurations, and sets
	 * post-game outcomes as needed.
	 * 
	 * This method does initial processing of the
	 * report string, then farms work out to several
	 * private methods for salvage, payment, etc.
	 * 
	 * Using the discrete private methods makes it
	 * a bit simpler to track all of the paramaters
	 * and variables in play ... also allows for
	 * automagical disconnection resolution.
	 */
	public void resolveShortAttack(Operation o, ShortOperation so, String report) {
		...
		/*
		 * Process the winner and loser lists. These should,
		...
		/*
		 * assemble the winner and loser strings, and
		 * the final status info string.
		 * 
		 * NOTE: This is where meta-impacts are applied.
		 *       See method for more detailed comments on
		 *       assignment of conquest %, thefts, etc.
		 */
		this.assembleDescriptionStrings(o, so);
		
		/*
		 * put together the salvage strings, and move units around. also
		 * determine cost of player's salvage. save these costs so they 
		 * may be used to adjust players' paystrings.
		 */
		this.assembleSalvageStrings(o, so);
		
		/*
		 * Put together the payment strings, and pay the players.
		 * Adjust the actual game pay by salvage & tech/bay costs.
		 */
		...
		/*
		 * Send messages to the players, remove them from fighting status,
		 * and inform them of any immunity they may have received.
		 */
		for (SPlayer currPlayer : allPlayers.values()) {
			
			//send messages
			String currName = currPlayer.getName().toLowerCase();
			String toSend = metaStrings.get(currName) + unitStrings.get(currName) + payStrings.get(currName);// + longStrings.get(currName);
			...
			MMServ.mmlog.resultsLog(toSend);
			 ...
			 * to avoid games, so they can scrap units without cost and reset in SOL. 
			 */
		...
		/*
		 * Set the game to finished staus
		 * and bump the game counter.
		 */
	...
	/**
	 * Method which resolves a short operation in the
	 * absence of an autoreport - namely, when one player
	 * disconnected from a game for an inordinately long
	 * period of time.
	 * 
	 * Instead of processing a complete autoreport, this
	 * resolution pathway rebuilds the state of the game
	 * at the time of the disconnection using a TreeMap
	 * which contains the status of all units removed from
	 * the game up to that point (destroyed, salvage, etc).
	 * 
	 */
	public void resolveShortAttack(Operation o, ShortOperation so, String winnerName, String loserName) {
		...
		/*
		 * Set up a Map with all of the players from the game. We
		 * know that only 2 players are involved; however, the
		 * allPlayers and allArmies maps are used in the salvage
		 * method and elsewhere, so this is needed to conform and
		 * use the same processing methods as an autoreport.
		 */
		allPlayers = new TreeMap<String, SPlayer>();
		allArmies = new TreeMap<String, SArmy>();
		...
		/*
		 * Put together the payment strings, and pay the players.
		 * Adjust the actual game pay by the salvage costs.
		 */
		...
		/*
		 * Unlock all participating armies. This may result in some
		 * oddities for players who are no logner connected, but its
		...
		/*
		 * Send messages to the winner, remove them from fighting
		 * status, and inform him of any immunity he may have received.
		 */
		
		//send message
		String winName = winner.getName().toLowerCase();
		String toSend = "Time expired. GAME RESOLVED AUTOMATICALLY.<br>" + metaStrings.get(winName) + unitStrings.get(winName) + payStrings.get(winName);// + longStrings.get(winName);
		...
		/*
		 * Send the message to the loser/disconnector. The player is offline,
		 * so their status and immunity time are not concerns at the moment.
		 */
		String loseName = loser.getName().toLowerCase();
		toSend = "You were disconnected too long. GAME RESOLVED AUTOMATICALLY.<br>" + metaStrings.get(loseName)  + unitStrings.get(loseName) + payStrings.get(loseName);// + longStrings.get(loseName);
		...
		//send to news feed, if the server ops believe this is a "meaningful" game
        if (o.getBooleanValue("ReportOpToNewsFeed"))
        	CampaignMain.cm.addToNewsFeed(newsFeedTitle,newsFeedBody);
	}
	
	/**
	 * Method which determines the payment which should be
	 * given to each player, adjusts the payment for salvage
	 * costs, and triggers the pay of techs.
	 */
	private void assemblePaymentStrings(Operation o, ShortOperation so, SPlayer disconnector) {
		...
		//load the base payments. these will be used as starting points for players.
		...
		//Building Payments.
		//make sure that it wasn't a building map that they fought on and the SO screwed up and set building ops happen.
		if ( buildingsLeft > -1 && buildingsLeft <= so.getTotalBuildings() ){
			...
			//attacker only gets paid if they destroyed the minimum amount of buildings.
		...
		/*
		 * Loop through all players in the set. Draw a name from the keyset and get the
		 * latest copy of the SPlayer from campaign main. This is less than ideal. Once
		 * we can guarantee that all SPlayers are updated as players leave/join. we can
		 * use the players directly.
		 * 
		 * Loading ops values in the loop looks inefficient; however, for 1 v 1 games in
		 * loop loads are actually more efficient than preloading - youre going to take
		 * a combination of 2 payment paths one time each, not all 4. Break even in team
		 * 2 v 2's, would be better off prefetching with large melees.
		 */
		...
		for(String currName : allPlayers.keySet()) {
			...
			/*
			 * If the player disconnected, tell him that he gets no
			 * payment or reduced payment, then continue normally.
			 */
			...
			/*
			 * Payment breaks into 4 primary situations, each
			 * of which has its own configurable modifiers.
			 * 
			 * 1 - winning attacker
			 * 2 - winning defender
			 * 3 - losing attacker
			 * 4 - losing defender
			 * 
			 * First determine whether player is an attacker
			 * or defender and set the base. Then check his
			 * winner/loser status and modify his payments.
			 */
			 ...
			 * eligibility for payment (SOL games) and adjust the cash for repair
			 * and salvage costs.
			 */
            ...
            //Check to make sure the battle did enough damage to reward everything.
            //Only the loser is affected and, for the time being, only money is reduced.
            ...
            /*
             * Determine how much to play the players technicians (or,
             * if using AR, the bay rental fee).
             */
            int techPayment = 0;
			...
			/*
			 * Put together the actual string to show our bold hero,
			 * and add the earned amounts to their accounts.
			 */
			...
			if (earnedMoney > 0 || salvageAndRepairCosts > 0 || battleLossCompensation > 0 || techPayment > 0) {
				...
				// if salvage costs are greater than the amount earned, reduce them.
				...
				if (availMoney < 0) {availMoney = 0;}
				
				/*
				 * If the tech payment is greater than the amount the player has available,
				 * including both the pay from this game and pre-game funds, some of the
				 * techs will quit or bays will be seized by their owners.
				 */
				int techsLost = 0;
				...
				/*
				 * If the player loses techs, set some units to be unmaintained and
				 * store a warning string to add to the final message.
				 */
		...
		if (allPlayers.size() == 2 && opEffectsElo && !drawGame ) {
			...
			/*
			 * Tasks used to give a larger rating change for
			 * big games, but it was an arbitrary unit count ...
			 * 
		...
		/*
		 * Add clickable defection link for SOL players, if
		 * they have enough XP to leave the training faction.
		 */
		for(String currName : allPlayers.keySet()) {
	...
	/**
	 * Method which determines player salvage and generates
	 * unique strings. Salvage costs are also determined and
	 * saved here; however, they are saved for use in the
	 * mathod which generates payStrings rather than shown
	 * directly to the players.
	 * 
	 * Determination as to which units are salvageable, which
	 * pilots survived the game, etc. should already have been
	 * made by one of two methods:
	 * - possibleSalvageFromReport(), or
	...
	private void assembleSalvageStrings(Operation o, ShortOperation so) {
		...
		/*
		 * Immediately check to see if this is a no-destruction game. If
		 * so, move ALL units from salvaged into living and clear out the
		 * pilots hash.
		 */
		...
		for (OperationEntity currEntity : livingUnits.values()) {
			...
			/*
			 * Null owners boggle my mind. Lets throw an error and look
			 * for some kind of pattern. Skip to the next unit and allow
			 * normal resolution.
			 */
			if (owner == null) {
				MMServ.mmlog.errLog("Null _owner_ while processing post-game salvage for " 
						+ " Attack #" + so.getShortID() + ". Needed to find Player: " + ownerName
						+ " Unit #" + currEntity.getID() + "/Type: " + currEntity.getType());
				continue;
			}
            ...
            try {
                ...
                MMServ.mmlog.errLog("Unable to apply damage to unit "+currU.getModelName() );
                MMServ.mmlog.errLog(ex);
            }
            
			//If damaged is transfered from Game to campaign then save it the pilot
			...
			//we may assume that a living unit earned kills, XP, or -somehow- changed. the number
			//of units whic hlive through a game without any change in their info is exceedingly
		 ...
		 * they are returned to their original owners. In this case, the pilots get
		 * kill XP, but do not gain any game-play XP.
		 */
		...
		for (OperationEntity currEntity : salvagableUnits.values()) {
			...
			/*
			 * See the comments for setupPilotString for array information.
			 * This is a fragile array - needs specific objects in specific
			 * indices, or reporting will fail.
			 */
			...
			if (winnerAlwaysSalvagesOwn && winners.containsKey(oldOwnerName)) {
				...
				/*
				 * Check the pilot's death. If he passed away, call the
				 * death handler and append the result to the owners'
				 * pilotString.
				 */
				...
				/*
				 * loop through all players, adding the unit's outcome
				 * to their salvage message trees. Send the owner his
				 * own message and set the other message for remaining
				 * players. Even though this is a winner salvaging
				 * his unit, we need to check captors - a pilot may have
				 * been picked up by an enemy on the field.
				 */
				for (String currName : allPlayers.keySet()) {
					
					SPlayer captor = (SPlayer)pilotinformation[4];
					String toSet = toOthers + pilotinformation[3];//default to otherstring 
					if (currName.equals(oldOwnerName))
						toSet = toOwner + pilotinformation[1];//set the "owner" string
					else {
				 ...
				 */
				int costToRepair = getSalvageCost(oldOwner, currU, currEntity, o, so);
				...
				/*
				 * Check to see if the player already has a scrap
				 * thread. If so, add this unit as an entry. If not,
				 * create a new OpsScrapThread for the player.
				 */
				...
				/*
				 * Nothing more to do with this particular unit in the 
				 * for each(OperationEntity : salvagable) loop. continue
				 * to next unit.
				 */
				continue;
			 ...
			 * attackers units, even in a loss, much to Kai's consternation),
			 * or we're dealing with a loser's unit. Either way, make a roll
			 * to determine who gains the unit itself.
			 */
			...
			/*
			 * Check the pilot's death. If he passed away, add
			 * the relevant info to the owner's string.
			 * 
			 * Only check for a replacement pilot if the unit is
			 * returning to its original owner!
			 */
			Boolean pilotLived = (Boolean)pilotinformation[0];
			...
			/*
			 * If the new owner is null, we know that the
			 * unit was returned to its previous owner.
			 */
			if (newOwner == null ) {
			...
			for (String currName : allPlayers.keySet()) {
				...
				/*
				 * determine which string to send. default to the
				 * "other" message, but check to see if the name is
				 * the same as the owner.
				 */
				SPlayer captor = (SPlayer)pilotinformation[4];
				String toSet = toOthers + pilotinformation[3];//default to otherstring 
				if (currName.equals(oldOwnerName))
					toSet = toOriginalOwner + pilotinformation[1];//set the "owner" string
			...
			/*
			 * If the newowner is not null, move the unit
			 * from its old master to the new. This will
			 * clear out the pilot if PPQs are in place.
			 * 
			 * If PPQs are not being used, get a new pilot
			 * for the unit out of the newOwner's house queue.
			 * 
			 * No PL|UU sent - unit is sent in string form by
			 * SPlayer.addUnit()
			 */
			if (newOwner != null) {
				...
				if (CampaignMain.cm.isUsingCyclops())
					CampaignMain.cm.getMWCC().unitChangeOwnerShip(Integer.toString(currU.getId()),newOwner.getName(),newOwner.getMyHouse().getName(),so.getOpCyclopsID(),"Salvage");
				...
				newOwner.addUnit(currU, true);//add unit is same as a PL|UU in most respects ...
				
				//add new pilot info to this unit's string
				String workingS = unitStrings.get(newOwner.getName().toLowerCase());
				workingS = workingS.substring(0,workingS.length() - 4);//remove "<br>"
				unitStrings.put(newOwner.getName().toLowerCase(), workingS + newPilot + "<br>");
			}
			
			/*
			 * Old owner. If the pilot lived throughthe game and grant him
			 * his kill XP, but do not attempt to level up.
			 */
			else {//old owner 
			...
			if (newOwner != null) {
				costToRepair = getSalvageCost(newOwner, currU, currEntity, o, so);
		...
		for (OperationEntity currEntity : destroyedUnits.values()) {
			...
			/*
			 * Check the pilot's death. If he passed away, do
			 * nothing. There's no unit to put a new pilot into.
			 * 
			 * If the pilot lived, put him in the house queue or
			 * the PPQ, as appropriate.
			 */
			Boolean pilotLived = (Boolean)pilotinformation[0];
			if (pilotLived.booleanValue())
				this.handleDispossesedPilot(oldOwner, currU);
			 ...
			 * or captured. Others are not informed of events,
			 * but Captors are given info on their pickups.
			 */
			for (String currName : allPlayers.keySet()) {
	...
	/**
	 * Method which generates general info strings (who attacked
	 * whom, which faction won, etc) for the players and a finished
	 * status description for public viewing.
	 * 
	 * After these general strings are put together, the need to
	 * check Operation values and apply meta effects (% transfers,
	 * unit thefts, etc) is checked.
	 * 
	 * If the given ShortOperation is capable of impacting factions
	 * (isn't a mult-faction melee, isn't tied to a long operation),
	 * the necessary values are loaded from the passed Operation and
	 * checked/applied.
	 * 
	 * Some have commented that this is too procedural and not very
	 * OO; however, it works. Anyone who cares to beautify the process
	 * is more than welcome to do so. @urgru 8.26.05
	 */
	private void assembleDescriptionStrings(Operation o, ShortOperation so) {
		...
		try{//main try for the whole function
			...
			/*
			 * Before we make strings, we need to determine
			...
			/* 
			 * Throw the entire meta-outcome out if the winner
			 * or defender trees include players from multiple
			 * factions - that would mean it was an arena or a
			 * duel.
			 * 
			 * These loops are heinously inefficient - (O)n^n -
			 * but we can pretty safely assume that n will never
			 * be larger than 4.
			 */
			...
			for (SPlayer lp : losers.values()) {
				for (SPlayer compareP : losers.values()) {
					...
					if (defendersPolluted) {break;}
				}
			}
			
			/*
			 * If there is pollution of the lists (not mono-faction) put together
			 * a melee string which shows the "teams" in play.
			 * 
			 * The code below is actually very simple, although the loops used to
			 * generate some of the strings don't read well and make it overly long.
			 * 
			 * NOTE: Complete and Incomplete info for Polluted games is IDENTICAL b/c
			 *       there is no faction affiliation to attach to the outcome.
			 */
			if (attackersPolluted || defendersPolluted) {
				...
				try {
					 ...
					 * "Player A defeated Player B and Player C (Arena)."
					 * 
					 * Also set up the generic strings at this time. The
					 * generics are used when generating the messages
					 * sent to individual players. Template:
					 * "You defeated Player B and Player C (Arena)", where
					 * "Player B and Player C" is the genetic loser string.
					 * 
					 * In p
					 */
                    ...
                    /*
					 * Because the winner/loser lists are polluted, we need
					 * to show all of the players names instead of simply
					...
					if ( !drawGame ){
						for (SPlayer wp : winners.values()) {
							...
							//else, send a plural message. ugly, but we need to not send the player his own name.
							else {
								int i = 0;
					...
					//copy the complete finished info to the incomplete, as they should be the same in a polluted game
					incompleteFinishedInfoString = completeFinishedInfoString;
					
					/*
					 * We don't want polluted games to have any
					 * impact on the player factions. Return.
					 */
					return;
				}
				catch (Exception ex){
			...
			/*
			 * If the winner and loser trees are nor polluted, generate
			 * 1 v 1 or faction strings, as appropriate. These are similar
			 * to the polluted meta strings; however, faction names replace
			 * individual player names in large games. 
			 * 
			 * Templates:
			 * "You defeated Player A on Solaris VII (TestAttack)."
			 * "Your team was defeated by Davion on New Avalon (Conquer)."
			 * "Your team defeated Liao on Sian (Raid)."
			 * 
			 * These should be fleshed out with more descriptive verbage
			 * in the future; however, this should suffice for the time
			 * being ...
			 */
			try{
				 ...
				 * has no long-backing, actual meta info will be tacked onto this.
				 * 
				 * Use a faction name in lieu of a player name in team games, or if
				 * the "ShowCompleteGameInfoInNews" option is disabled.
				 */
			 ...
			 * post-game faction impacts. The status of the long op to
			 * which the ShortOperation must be bound isn't check until
			 * after salvage resolution.
			 */
			 ...
			 * have games which are components of LongOperations. Any ShortOp which
			 * remains at this point may be configured to have faction-impacts.
			 * 
			 * Its entirely possible that none will be enabled (ie - 1 v 1 duel
			 * for cash/ELO); however, still need to check and apply if necessary.
			 * 
			 * Check to see whether the attacking group or defending group won. Only
			 * load the variables from Operation that we actually need.
			 */
			...
			try{
				...
				//caps are used by both attackers and defenders, so load them here
				...
				//queue updates to send to houses that may be affected by the results.
				...
				if (attackersWon) {
					try{
						/*
						 * Determine the amount of land which should change hands
						 * as a result of the Operation. Check to make sure there
						 * is actually land to gain - an attacker who already owns
						 * 100% of a world should gain nothing.
						 */
						...
						//make the % adjustment. This zeros totalConquest if there is nothing to gain.
						...
						/*
						 * Determine the amount of delay to apply to the facilities on world.
						 * Note that delay is applied to *all* facilities under Operations,
						 * whereas under Tasks is was applied to one random facility on world.
						 */
						...
						//not allowed to delay or steal from factories the defender doesn't own
						if ( so.getTargetWorld().getOwner() == null ||
								!so.getTargetWorld().getOwner().getName().equalsIgnoreCase(aLoser.getMyHouse().getName()) ){
							totalDelay = 0;
						}
						...
						//if no facilities exist to delay, reduce the total to 0.
						...
						if (totalDelay > 0) {
							...
							while (i.hasNext()) {
								SUnitFactory currFacility = (SUnitFactory)i.next();
							...
							loserMetaString += " allowed " + aWinner.getHouseFightingFor().getColoredNameAsLink() + " to stop production on " +
							target.getNameAsColoredLink() + " for " + totalDelay + " miniticks";
							hasLoss = true;
						 ...
						 * but also save their model info in an ArrayList of Strings. The AList will
						 * be used later to generate messages for players and the finishedInfo.
						 * 
						 * Operations gives all stolen units to the house. None go to the player. This
						 * is a major deviation from the old Task system; however, giving units to the
						 * players makes very little sense when payments can be increased and games can
						 * result in several different kinds of outcomes.
						 * 
						 * Also - If there are no units of a proper type (ie - no factory making units
						 * on the world or no units of types which factories DO make), little it taken.
						 * Rollover curtailments are in place only ShortOperations. LongOps make provision
						 * for rollover plus-ups, such that the class/type of units taken is increased if
						 * none remain in the target type. This is to discourage factions from clearing
						 * out bays in response to a multi-game theft operation - not a situation which
						 * arises frequently with shorts. This, they have downward/reduced spillover.
						 */
						...
						if (unitsToCapture > 0) {
							 ...
							 * we've taken what we can. This may mean getting inf
							 * or vehs on a planet than can produce assault mechs.
							 */
							...
							while (factoriesSearched.size() < target.getFactoryCount()) {
								...
								//if we've already searched this factory before, skip
								if (factoriesSearched.contains(currFacility))
									continue;
								
								//we've not searched the facility before. add it to the searchlist
								...
								for (int type = Unit.MEK; type <= Unit.BATTLEARMOR; type++) {
									...
									}//end while(units remain in this factories' pool)
								}//end for(all types)
						...
						/*
						 * Determine the number and type of components to steal from target
						 * faction. Much like Unit theft, this is handled in a different
						 * manner than it was under the Task system.
						 * 
						 * If a defined target is given (specific factory), take only from that
						 * target. If nothing is there, no rollover. If no factory is given,
						 * select random factories until the PP amount is reached or all
						...
						//not allowed to delay or steal from factories the defender doesn't own
						if ( so.getTargetWorld().getOwner() == null || !so.getTargetWorld().getOwner().getName().equalsIgnoreCase(aLoser.getMyHouse().getName()) ){
							ppToCapture = 0;
						}
						 ...
						 * Try every factory on the world, at random, until
						 * we've taken what we can. This may mean getting inf
						 * or veh pp on a planet than can produce meks.
						 * 
                        ...
                        /*
                         * Determine the number and type of components to destroy from target
                         * faction. Much like Unit destruction, this is handled in a different
                         * manner than it was under the Task system.
                         * 
                         * If a defined target is given (specific factory), take only from that
                         * target. If nothing is there, no rollover. If no factory is given,
                        ...
                        int ppDestroyed = 0;
                        factoriesSearched.clear();
                        while (factoriesSearched.size() < target.getFactoryCount() && ppDestroyed < ppToDestroy) {
				...
				//DEFENDER VICTORY BLOCK
					...
					try{
						...
						/*
						 * Determine the amount of refresh to add to the facilities on world. This
						 * is an inverse of what the attacker was trying to get - bonus miniticks
						 * for the factories.
						 */
						...
						//if no facilities exist to refresh, reduce the total to 0.
						if (target.getFactoryCount() == 0)
							totalRefreshBoost = 0;
						
						//not allowed to boost production or decrease factory delays if the defender doesn't own
						if ( so.getTargetWorld().getOwner() == null ||
								!so.getTargetWorld().getOwner().getName().equalsIgnoreCase(aWinner.getMyHouse().getName()) ){
							totalRefreshBoost = 0;
						}
						...
						if (totalRefreshBoost > 0) {
							...
							loserMetaString += " encouraged the workers on " + target.getNameAsColoredLink() +
							" to speed up the production lines for " + totalRefreshBoost + "miniticks";
							hasLoss = true;
						...
						/*
						 * NOTE: This is the point where unit thefts appear in the attacker
						 * win block. Defenders cannot steal or insta-build units, so its
						 * been removed. 
						 */
						
						/*
						 * Determine the number and type of components to produce. The
						 * defender cannot choose which type of components should be
						 * produced. They will always be generated at random from the
						 * factories available on world.
						 * 
						 * We only need to do this if the target actually has factories.
						 * If it doesnt, the attacker was wasting his time anyway ...
						 */
						if (target.getFactoryCount() > 0) {
				...
				//only futz with the metastring additions if there is actual loss.
				if (hasLoss) {
					
					/*
					 * Strip the the last instance of ", " and insert " and " in order
					 * to have a properly formatted metastring. After correcting the
					 * format, append to the finishedInfo and add to player's entries
					 * in the metaStrings tree.
					 */
					String meta1 = "";
					String meta2 = "";
	...
	/**
	 * Method which parses salvage from the report
	 * and saves outcome strings for the players.
	 */
	private void possibleSalvageFromReport(StringTokenizer reportTokenizer, ShortOperation so) {
		...
		try{//main catch, make new lists and a new pilots hash.
			...
			try{
				...
				while (reportTokenizer.hasMoreTokens()) {
					...
					if (currentUnit.startsWith("MW*")) {
						...
						mwTokenizer.nextToken();//burn the second token. always -1. unused.
					...
					//check for builings left this is tacked onto the end of the results string.
					...
					//Combat Unit. Make and store an OperationEntity.
						...
						//if the player doesnt own the unit, its probably autoartillery. continue to next loop.
						if ( unit == null)
							continue;
						...
						if (oEntity.isLiving()){
                            ...
                            destroyed = 1;
                            /*
                             * I've changed the code so that only the BV of living units is reported.
                             * if the unit was destroyed(but not cored) or forced salvage they do not
                             * get a BV count. Might tweak this later --Torren.
                             */
                            //currentBV += unit.getBV();
                        }
						else{
                        ...
                        /*
                         * Count all real games for mechstats. Do NOT count disco
                         * resolutions as they are not meritorious.
                         */
                        if (saveStats) {
				...
				//final game BV determined. save to the short op for use in /c modgames.
			...
			/*
			 * Need to use an iterator here b/c it allows for a
			 * chance to remove units (over run) from living units,
			 * which foreach does not.
			 */
			...
			try{
				while (i.hasNext()) {
					...
					//integer which reresents this OEntity's likelyhood of being overrun
	...
	/**
	 * Method which builds possible salvage from a Short
	 * Operation's death tree. Since the information stored
	 * while a game is in progress is less exact than a full
	 * autoreport, some assumptions must be made.
	 * 
	 * By setting up accurate living, salvage and dead unit
	 * treemaps, this method allows the resolver to use its
	 * remaining methods (salvage assignment, etc) as usual.
	 */
	public void possibleSalvageFromInProgressInfo(ShortOperation so, SPlayer loser) {
		try{
			...
			pilots = new TreeMap<Integer, SPilot>();
			try{
				//populate the units trees using removed units. some may end up in living.
			...
			/*
			 * Build a vector of units which are living and belong to the
			 * player who dropped from the server.
			 */
			...
			/*
			 * If the dropper doesnt have any living units, we move on and
			 * do not apply a penalty. If he had pieces survive the game,
			 * check server settings to see if additional units should be
			 * destroyed or set as salvage.
			 */
			try{
				if (dropLivingUnits.size() > 0) {
					
					int unitsToDestory = CampaignMain.cm.getIntegerConfig("DisconnectionAddUnitsDestroyed");
					int unitsToSalvage = CampaignMain.cm.getIntegerConfig("DisconnectionAddUnitsSalvage");
					
					//DESTORY! RAWR!
					int unitsDestroyed = 0;
					while (unitsDestroyed < unitsToDestory && dropLivingUnits.size() > 0) {
						
						OperationEntity randomEntity = dropLivingUnits.remove(CampaignMain.cm.getR().nextInt(dropLivingUnits.size()));
	...
	/**
	 * Method which cleans up when a pilot dies. Determines whether
	 * or not peronal queues are in use, re-BV's the unit and the
	 * armies in which it is resident, and returns a string detailing
	 * the outcome.
	 */
	...
	/**
	 * Method which handles living, dispossessed pilots. Called
	 * from the salvage method. Very simple. Broken out to slice
	 * repetetive code.
	 */
	private void handleDispossesedPilot(SPlayer owner, SUnit unit) {
		...
		try{
			...
			boolean isPilotChangeable = (unit.getType() == Unit.MEK 
					|| unit.getType() == Unit.QUAD 
					|| unit.getType() == Unit.PROTOMEK);
	...
	/**
	 * Method which checks the status of a pilot. Called when
	 * checking on salvaged and destroyed units. 
	 * 
	 * If the pilot is non-null, can check the is dead field
	 * for status and reason for showing on field.
	 * 
	 * If the pilot is null, death is presumed. Can derive the
	 * cause of death from the status of his unit.
	 * 
	 * This method returns an array which has very specific
	 * object ordering, as follows:
	 * 
	 * 0 - Boolean pilotIsDead; indicates status of pilot
	 * 1 - String ownerString;  to send to the original owner
	 * 2 - String captorString; to send to capturing player
	 * 3 - String otherString;  to send to player who are neither
	 *                          original owners nor captors
	 * 4 - SPlayer captor;      player who has captured a pilot
	 */
		...
		try{
			...
			if (mw != null) {
				...
				if (pickupPlayer != null) {
					...
					//if the player is fighting for the same house, simply return the pilot
					...
					//player isn't friendly, so we can set the messages to the owner ...
					...
					if (CampaignMain.cm.getR().nextInt(100) < captureChance){
						...
						toReturn[0] = new Boolean(false);//although the pilot is technically alive, treat as dead
				...
				/*
				 * The pickup was null, but the pilot was not. Check to see
				...
				//didn't survive. can set up the message to owner and select a random pickup player
				...
				if (CampaignMain.cm.getR().nextInt(100) < captureChance){
					if (CampaignMain.cm.getBooleanConfig("AllowPersonalPilotQueues")){
						...
						pickupPlayer.getHouseFightingFor().addDispossessedPilot(currUnit, true);
					toReturn[0] = new Boolean(false);//pilot technically lives, but we say that he is dead.
					toReturn[2] = ((SPilot)currUnit.getPilot()).getPilotCaptureAndDefectedMessage(currUnit,pickupPlayer.getMyHouse());
					return toReturn;
				} 
			...
			/*
			 * The pilot is null. This generally means that the pilot is dead; however,
			 * it may also mean that the unit was killed in a way which doesnt trigger
			 * an ejection - generally 3 engine crits - of the unit simply doesn't have
			 * an ejectable pilot (proto, BA).
			 */
			...
			if (currEntity.getType() == Unit.PROTOMEK) {
				...
				/*
				 * If the vehicle is salvageable and the crew is alive, we have to test for
				 * winner and loser, same as for a pilot trapped in an engine-destroyed mek.
				 */
				if (currEntity.isSalvage() && !currEntity.isCrewDead()) {
					...
					//always give them a fighting chance. no survivalist adjustment.
					int minChance = CampaignMain.cm.getIntegerConfig("BasePilotSurvival");
					if (survivalChance < minChance)
						survivalChance = minChance;
				...
				/*
				 * If the vehicle is salvageable and the crew is dead, set
				 * an appropriate message and resolve salvage normally.
				 */
			...
			//see if the location containing the cockpit was obliterated (HD or CT)
			if (currEntity.getHDint() <= 0 && currEntity.getCockpitType() != Mech.COCKPIT_TORSO_MOUNTED 
			...
			/*
			 * If pilots in downed units need to roll for survival, check them. This code is
			 * virtually identical to the capture block when a pilot is left alive on field
			 * and could probably be factored out into a seperate checksurvival method...
			 */
			...
			/*
			 * Unique to in-mech pilots (engine kills). Penalty
			 * for being in a stationary unit when the capture
			 * crews come around and sweep the field.
			 */
			survivalChance -= CampaignMain.cm.getIntegerConfig("TrappedInMechSurvivalMod");
			...
			if (CampaignMain.cm.getR().nextInt(100) < captureChance){
				if (CampaignMain.cm.getBooleanConfig("AllowPersonalPilotQueues")){
					...
					pickupPlayer.getHouseFightingFor().addDispossessedPilot(currUnit, true);
				toReturn[0] = new Boolean(false);//technically alive, but dead for New Pilot purposes
				toReturn[2] = ((SPilot)currUnit.getPilot()).getPilotCaptureAndDefectedMessage(currUnit,pickupPlayer.getMyHouse());
				return toReturn;
			} 
	...
	/**
	 * Method which returns the cost of salvaging a given unit. A
	 * unit costs less to salvage after a headkill, more to salvage
	 * if torsos are destroyed, etc.
	 */
	private int getSalvageCost(SPlayer owner, SUnit u, OperationEntity oe, Operation o, ShortOperation so) {
		...
		try{
			...
			//head kills & ejections are easier to repair than most. reduce cost.
			if (oe.getHDint() <= 0)
				repairCost *= 0.7;
			...
			//capturing an enemy unit will be slightly more expensive then recovering one's own
	...
	/**
	 * Method which determines how much, if any, compensation a faction will give
	 * a player after a unit is lost (to either irreparable damage or an enemy) in
	 * a game. Flat/Percentage Caps and unit type adjustments allow admins to give
	 * greater amounts (relatively) for smaller units, meks, and so on.
	 */
	private int determineLossCompensation(SPlayer p, SUnit u, boolean salvage) {
		...
		int newPrice = p.getMyHouse().getPriceForUnit(u.getWeightclass(), u.getType());
		...
		/*
		 * If the unit was salvaged by an an enemy, instead of destroyed, check for a salvage multiplier. We
		 * will usually want to reduce the payment (giving away equipment, etc), but some servers may want to
		 * boost the payout to encourage non-destructive play.
		 */
		if (salvage)
			compensation *= CampaignMain.cm.getFloatConfig("SalvageMultiToUnitLossPayment");
		
		/*
		 * Apply specific type multipliers. Can be used to reduce the compensation for a
		 * class of units. Useful, for example, to give hih compensation for Light Meks,
		 * but reduced compensation for light vehicles.
		 */
		...
		//check the compensation caps. 1st check reduces compensation to a portion of a new unit's cost.
	 ...
	 * the winner tree. Used to assign salvage and
	 * on-field pilot captures.
	 */
	private SPlayer selectRandomWinner() {
		...
		try{
			...
			/*
			 * We have a tree with more than one person. Get the
	...
	private void contractFinishedInfo(SPlayer player){
		...
		if ( duration <= contract.getEarnedAmount() ){
			...
			CampaignMain.cm.toUser("You completed your contract with "
					+ contract.getEmployingHouse().getName() + " and received the final payment ("
					+ CampaignMain.cm.moneyOrFluMessage(true,true,payoff,false) +").",player.getName(),true);
    ...
    private String calculatePilotEXP(Operation o, ShortOperation so, OperationEntity currEntity, SPlayer owner, boolean allowLevelUp){
        ...
        if (currEntity.getKills().size() > 0 && countKills) {
            ...
            for (Integer id : currEntity.getKills()) {
                ...
                //if the owner doesnt have it, its probably an autoassigned unit or mechwarrior.
