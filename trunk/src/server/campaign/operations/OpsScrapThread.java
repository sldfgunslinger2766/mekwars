...
public class OpsScrapThread extends Thread {
    ...
    public void setMaxPayment(int maxPayment) {
        ...
        /*
         * When max is set, loop through salvaged units and cap their scrap values such that none pays more than the maximum.
         */
    ...
    public void scrapUnit(int unitID) {
        ...
        /*
         * Now that we have a real value, add it to the payments to date and check to ensure that other units in the tree are not going to cross the pay ceiling if they are scrapped. Reduce the scrap value of any unit which would breach the cap, and send a PL|UU to ensure that the owner's GUI shows the same.
         */
        ...
        /*
         * Check to see that the player still owns the replaced units (hasn't donated or transferred or sold). If so, change their values and send a PL|UU.
         */
        for (int currID : toReplace.keySet()) {
    ...
    public synchronized void run() {
        ...
        /*
         * Fetch the player. If he's null or logged out, he's presumably quit or disconnected. He doesn't need any PL|UU's or the opportunity to scrap for cash.
         */
        SPlayer currPlayer = CampaignMain.cm.getPlayer(playerName);
        if (currPlayer == null) {
            return;
        ...
        /*
         * Wait is over. Refresh the player (in case he logged out and then returned) and check the number of units left in the scrapmap. If he still has some of the units, 0 their scrap values and send updates.
         */
        if (isFinished) {
            return;
        }
