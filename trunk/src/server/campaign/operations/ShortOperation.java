...
public class ShortOperation implements Comparable {
    ...
    //Starting values. Used in /c modgames and in ShortResolver. Increased by addAttacker/Defender.
    int startingBV = 0;
    int startingUnits = 0;
	 ...
	 * code is already multiplayer friendly.
	 */
	...
	/*
	 * The GameOptions and Attacker/Defender AUtoArmies. Generated
    ...
    /* 
     * Building Options
     * for any building destruction tasks
     */
    ...
    private int[] teamEdge = {Buildings.NORTH,Buildings.SOUTH,Buildings.EAST,Buildings.WEST,Buildings.NORTHWEST,Buildings.SOUTHEAST,Buildings.NORTHEAST,
    						Buildings.SOUTHWEST};
	...
	/*
	 * last, but certainly not least, holders for the underlying
	 * operation and (if extent) player ModifyingOperation names.
	 * 
	 * use names instead of direct references. Resolver will be fed
	 * latest version of named ops/mods by Manager when game ends.
	 */
    ...
    //CONSTRUCTOR
	public ShortOperation (String opName, SPlanet target, SPlayer initiator, SArmy attackingArmy, ArrayList<SArmy> possibleDefenders,
	...
	//METHODS
	/**
	 * Method which adds an attacker to the short.
	 * Should only be called after validation.
	 * 
	 * NOTE: Ops store a player's NAME, not a special
	 * player like the old TaskPlayer. Instead of a
	 * special player. The ID# of the army used in the
	...
	/**
	 * Method which removes an attacker, and all of his
	 * armies, from a ShortOperation.
	 */
	...
	/**
	 * Method which adds a defender to the short.
	 * Should only be called after validation.
	 */
	public void addDefender(SPlayer p, SArmy a, String modName) {
		 ...
		 * are reached, unless the attacker launches the task sooner.
		 * If not a team game then check for FFA
		 */
	...
	/**
	 * Method which returns the name of this operation.
	 * Used to pull an Operation (paramater bag) from
	 * the manager. 
	 */
	...
	/**
	 * Method which returns the map of
	 * attacking SPlayers.
	 */
	...
	/**
	 * <b>Note:</b> Any variables added here that are sent to the player 
	 * must be added to <b>sendReconnectInfoToPlayer</b>
	 * 
	 * <br>Method which changes the status of the Operation. This
	 * is analagous to the switchTo...() methods in the old Task
	 * class.
	 * 
	 * <br>Absolutely massive method :-(
	 * 
	 * <br>Should the contens here be moved to the OperationManager?
	 * 
	 */
	public void changeStatus(int newStatus) {
		
		/*
		 * Never change to waiting mode. First
		 * actual switch is to INPROGRESS.
		 */
		
		/*
		 * INPROGRESS change is analagous to Task's
		 * switchToRunning. It sends server options to
		 * the players, generates autoarmies and generally
		...
		if (newStatus == STATUS_INPROGRESS) {
            ...
            /*
             * Building ops always assign edged. If this is not a building op, check to
             * see if edges should be assigned to players. If not, we can assume that the
            ...
            //Make sure every position is filled before we go through and give duplicate positions.
            //This way if you have 9 or less players each gets it own starting position
            //More then 9 and you start doubling up.
            else if ( o.getBooleanValue("FreeForAllOperation") 
                    && this.getAllPlayerNames().size() > 2 ){ //2 player FFA start on opposite ends
                ...
                for (String currN : this.getAllPlayerNames()) {
                    ...
                    //Position 9 is edge so we are bumping it up one to 10 for CTR(center)
                    if ( pos == 9 )
                        pos++;
            ...
            /*
			 * Check to see if autoartillery should be
			 * assigned to either group of players. If
			 * so, check the group modifiers and boundaries,
			 * and then assign the tubes.
			 */
            ...
            /*
             * Check to see if mines should be
             * assigned to either group of players. If
             * so, check the group modifiers to find out how many.
             */
            ...
            //if we're going to be adding mines to someone, total the BVs in use and tonnage
            totalBV = 0;
            int totalTonnage = 0;
			 ...
			 * 1) Set busy status
			 * 2) Cancel any outstanding chicken threads
			 * 3) Add the game info to client-side logs
			 */
			...
			for (String currN : this.getAllPlayerNames()) {
				...
				CampaignMain.cm.sendPlayerStatusUpdate(currP,true);//send fighting info to all
			...
			/*
			 * Now that the Operation is actually running, it will
			 * return nifty/useful info. We want to show that info
			 * to the houses of all attacking/defending players, but
			 * don't want to spam housechannels if there is more than
			 * one participant per house. Set up a quick TreeMap in
			 * order to filtouer out duplicates, then send to all
			 * involved houses.
			 */
			...
			/*
			 * Send logos, intel reports and detailed game info to the
			 * invovled players. Logos are those of the first player
			 * in the attacker and defender maps, respectively.
			 */
			...
			//average the attacker and defender rankings, then compare for an intel report.
			 ...
			 * for awhile and are/were barely functional. Leaving player config'ed
			 * options out for now, but when they get rewritten, this is where
			 * they should be matched and sent to the players.
			 */
		...
		/*
		 * Setting to Reporting status doesn't do
		 * anything directly; however, it does stop
		 * additional autoreports from being processed.
		 */
		 ...
		 */
			 ...
			 * starting a game, sift through the factions to avoid spam'ing
			 * a housechan with duplicate entries.
			 */
			TreeMap<String, SHouse> houseMap = new TreeMap<String, SHouse>();
			for (String currN : this.getAllPlayerNames()) {
				SHouse currH = CampaignMain.cm.getPlayer(currN).getMyHouse();		
			...
			//check to see if complete or incomplete info should be shown to housemates
			String toSend = "";
	...
	/**
	 * Method which sends all game information - autoarty,
	 * map info, planet info, game options, etc. Used when
	 * a participant disconnects and then returns to the
	 * campaign server.
	 */
	public void sendReconnectInfoToPlayer(SPlayer p) {
		 ...
		 * resent in order to be properly loaded into MegaMek (autoarty, etc). Most
		 * of what needs to be sent was saved during changeStatus(IN_PROGRESS)
		 */
		if (currentStatus != STATUS_INPROGRESS)
			return;
		
		/*
		 * Lock info is cleared if the player is purged from memory while offline, so
		 * make sure that the player's army is still locked and send the lock status.
		 */
	...
	/**
	 * Method which returns the maxattackers. Generally
	 * used by JoinAttackCommand to make sure an additional
	 * attacker is still allowed.
	 */
	...
	/**
	 * Method which returns the maxdefenders. Generally
	 * used by DefendCommand to make sure an additional
	 * defender is allowed.
	 */
	...
	/**
	 * Method which returns the mindefenders. Used to
	 * ensure that we have enough defenders before
	 * cahnging to RUNNING status.
	 */
	...
	/**
	 * Method which sets the short ID # of this op. This
	 * should be called ONLY from  the Manager.
	 */
	...
	/**
	 * Method which returns underlying long ID #
	 * of this op. Note that -1 is sent to constructor
	 * if no longop underpins.
	 */
	...
	/**
	 * Method which returns the death tree. Should be called only from
	 * the ShortResolver and only when resolving a disconnection.
	 */
	...
	/**
	 * Method which returns running info on pilots. Should only be called
	 * from the ShortResolver and only when completing a dropped game.
	 */
	 ...
	 * 
	 * The chicken threads send defense links to players when they
	 * are started. We can assume that the waitingInfo is a good.
	 * 
	 * It might be more logical to set up the chicken threads in
	 * the manager, from an OO standpoint, but this allows the
	 * Operation to easily terminate its chicken threads when its
	 * status is changed from WAITING to RUNNING, after a /c term
	 * command is sent by the attacking player, etc.
	 */
	public void informPossibleDefenders() {
		...
		/*
		 * Now that we've set up all the possible threads and accounted
	...
	/**
	 * Method which removes a potential defender from the chicken
	 * tree. The term flag is set on the thread, then it is removed
	 * from the chicken tree. If necessary, the player is penalized.
	 * 
	 * This is generally called from within the ShortOperation, when
	 * it moves to RUNNING status, from DeactivateCommand when a player
	 * leaves active status, or from a ChickenThread (via SPlayer) when
	 * it deactivates a player.
	 */
	public void removePossibleDefender(String name, boolean penalize) {
		...
		if (penalize) {currT.doPenalty();}
		currT.stopChicken();
		chickenThreads.remove(currName);
	}
	
	/**
	 * Method which is called when a ShortOp is moved
	 * to running status or terminated. Completely closes
	 * the chickenTree.
	 * 
	 * Should only be called internally (status) or
	 * from OpsManager (term command, or noattacker/nodef).
	 */
	...
	/**
	 * Method which returns information (string) on this
	 * ShortOperation. Intelligently determines which type
	 * of info to send by looking at status.
	 * 
	 * NOTE: This method is public. Other info methods are
	 * private.
	 */
	...
	/**
	 * Private method which generates the first portion of a
	 * notification message for defenders. This is sent to
	 * chickenThreads as a constructor paramater. They complete
	 * the message by adding relevant /c defend# links.
	 * 
	...
	private String getDefenderMessage()  {
		...
		if (numAttackers == 1) {
			...
			resultString += attackArm.getInaccurateDescription();//show same for complete and incomplete, for now?
		}
	...
	/**
	 * Method which returns info for an operation
	 * which is waiting for players to join.
	 */
	...
	/**
	 * Method which returns info for an operation
	 * which has finished. String set by Resolver.
	 */
	...
	/**
	 * Method which is used (by Resolver) to set
	 * the complete finishedString for post-game display.
	 */
	...
	/**
	 * Method which is used (by Resolver) to set
	 * the incomplete finishedString for post-game display.
	 */
	...
	/**
	 * Method which may be used to set up attacker strings.
	 * NOTE: Private.
	 * 
	 * boolean - complete info
	 * boolean - always show name (used by in-progress)
	 */
	private String getAttackerString(boolean complete, boolean showName) {
		...
		if (complete && !blindDrop) {
			Iterator i = attackers.keySet().iterator();
			while (i.hasNext()) {
	...
	/**
	 * Method which may be used to set up defender strings
	 * NOTE: Private
	 */
	private String getDefenderString(boolean complete, boolean mod) {
		...
		if (mod || (complete && !blindDrop)) {
			Iterator i = defenders.keySet().iterator();
			while (i.hasNext()) {
	...
	/**
	 * Method which loops through the player
	 * collection looking for a given SPlayer.
	 */
	...
	/**
	 * Method which loops through the attacker and 
	 * defender maps in order to determine if any of
	 * the participating players is from a given house.
	 */
	...
	/**
	 * Method which determined whether or not the
	 * operation involves a player whose faction
	 * name begins with a given string.
	 * 
	 * Used by GamesCommand to filter output. We
	 * assume that s is all lowercase, b/c Games
	 * lowercases its faction filter @ parse.
	 */
	...
	/**
	 * Method which returns a collection of 
	 * all players involved in the Operation.
	 * 
	...
	/**
	 * Method which decrements) the showsToClear counter. Called on ticks. If the counter
	 * drops below 0, return true (id should be released), otherwise return false and show
	...
	/**
	 * Method which returns the completion time. Used
	 * to age-sort shortops in /c games. 
	 */
	...
	/**
	 * compareTo required for compliance with
    ...
    /**
     * Gets the total bv for the Combined Attacking forces. This is check
     * to start the attack.
    ...
    /**
     * Loop through all the defending teams and see if they are all ready to launch
     * They must have either the Max number of Team members or meet the Max Defender BV
     * or both for to the Operation to launch.  This code should allow multiple teams to
     * defend against a single team attack.
     * @param teams
     * @param players
     * @return
     */
